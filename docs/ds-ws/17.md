...
14.  接下来，我们为资产创建 ID。正如我们前面解释的，img/ 被映射到`housing`变量，有两个可能的值:`yes`或`no`。在代码的第一行，我们将变量`AssetId`的所有值初始化为`0`。在第二行中，只要Shell变量为`yes`，我们就将`AssetId`的值更改为`1`。这可以按如下方式实现:
