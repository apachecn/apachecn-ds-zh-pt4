...
>>> cumulative_returns = \
...     all_img/_analyzer.analyze('cumulative_returns')
...
![Figure 7.21 – Cumulative returns for all img/
](img/Figure_7.21_B16834.jpg)
...
最后，对于 ARIMA 和线性回归模型，我们希望可视化预测中的误差，或**残差**。拟合的模型都有一个`resid`属性，它将给出残差；我们只需要将它们绘制成散点图来检查它们的方差，绘制成 KDE 来检查它们的均值。为此，我们将添加`plot_residuals()`方法:
...
](img/Figure_7.27_B16834.jpg)

图 7.27-评估线性回归模型的残差

现在让我们看看 ARIMA 模型或线性回归模型在预测网飞股票的收盘价方面表现更好。
