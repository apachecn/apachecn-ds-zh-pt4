...
让我们再创建一个数据收集应用程序。我们可以在`requirements.txt`中指定我们需要`requests`库。我们也可以从 [第 15 章](eb2254cc-485d-4603-9d17-7a442aa5e3b8.xhtml)、*用Poetry和 PyTest* 打包测试中复制粘贴`_get_data`函数，以及资源和时间列。我们仍然缺少的一部分代码是上传数据到 S3。代码如下:
