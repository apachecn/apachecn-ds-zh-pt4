<title>C15019_07_Final_ePub_SP</title> <link href="css/epub.css" rel="stylesheet" type="text/css">
...
`GridSearchCV`将获取一个模型和参数，并为每个参数排列训练一个模型。在培训结束时，它将提供对参数和模型分数的访问。这被称为超参数调谐，你将在*第 8 章，超参数调谐*中更深入地了解这一点。
...
网格搜索是超参数调谐的第一批技术之一。然而，随着搜索空间的增大，它很快变得昂贵。随着参数选项的增加，搜索空间也会增加，因为参数选项的每种可能组合都会被考虑。
