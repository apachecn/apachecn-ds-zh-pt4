...
为了做到这一点，我们将使用**Beautiful Soup 4**(**BS4**)，它是解析 HTML 的两个主要库之一，与 **LXML** 一起使用。BeaultifulSoup还知道如何解析 HTML，甚至可以修复无效文件。一旦文档有了 Pythonic 表示，我们就可以通过使用元素 ID、类、CSS 属性、它们的顺序等等的组合，使用 CSS 选择器或 XPath 小型语言，深入并检索我们感兴趣的特定元素。
...
3.  BeaultifulSoup到底是做什么的？没有它我们能刮吗？
4.  为什么我们在这里使用递归？
5.  在刮除过程中应该清理数据吗？
