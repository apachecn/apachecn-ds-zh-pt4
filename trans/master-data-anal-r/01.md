...
![Loading text files of a reasonable size](graphics/2028OS_01_01.jpg)
...
<title>Benchmarking text file parsers</title><link href="epub.css" rel="stylesheet" type="text/css"> 
...
<title>Loading a subset of text files</title><link href="epub.css" rel="stylesheet" type="text/css"> 
...
生成的数据框仅包含原始数据集中 227，496 个案例中的 3，481 个观察值，当然，在临时 SQLite 数据库中进行过滤会稍微加快数据导入速度:
...
<title>Loading data from databases</title><link href="epub.css" rel="stylesheet" type="text/css"> 
...
![Setting up the test environment](graphics/2028OS_01_02.jpg)
...
这种要在任何计算机上运行的预先配置的虚拟机可以以多种文件格式从互联网上的各种提供商下载，例如 OVF 或 OVA。通用`VirtualBox`虚拟设备可以从[http://virtualboximages.com/vdi/index](http://virtualboximages.com/vdi/index)或[http://virtualboxes.org/images/](http://virtualboxes.org/images/)下载。
...
![MySQL and MariaDB](graphics/2028OS_01_03.jpg)
...
![PostgreSQL](graphics/2028OS_01_04.jpg)
...
![Oracle database](graphics/2028OS_01_05.jpg)

点击 **导入**后，您必须再次同意软件许可协议。导入虚拟磁盘映像(15 GB)可能需要几分钟时间:

![Oracle database](graphics/2028OS_01_06.jpg)

导入完成后，我们应该首先更新网络配置，这样我们就可以从外部访问虚拟机的内部数据库。所以让我们在设置中从 **NAT** 切换到**桥接适配器**:

![Oracle database](graphics/2028OS_01_07.jpg)
...
![Oracle database](graphics/2028OS_01_08.jpg)
...
![Oracle database](graphics/2028OS_01_09.jpg)
...
![ODBC database access](graphics/2028OS_01_10.jpg)
...
![Using a graphical user interface to connect to databases](graphics/2028OS_01_11.jpg)

在提供了所需的连接信息之后，我们可以轻松地查看原始数据和列/变量类型，并运行定制的 SQL 查询。一个基本的查询生成器也可以帮助新手用户从数据库中获取子样本:

![Using a graphical user interface to connect to databases](graphics/2028OS_01_12.jpg)
...
<title>Importing data from other statistical systems</title><link href="epub.css" rel="stylesheet" type="text/css"> 
...
<title>Loading Excel spreadsheets</title><link href="epub.css" rel="stylesheet" type="text/css"> 
