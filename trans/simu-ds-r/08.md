...
![Motivation to use the bootstrap](img/B05113_08_01.jpg)
...
![Motivation to use the bootstrap](img/B05113_08_02.jpg)
...
![Motivation to use the bootstrap](img/B05113_08_03.jpg)
...
回归模型![The most popular but often worst method](img/B05113_08_21.jpg)的估计在本节中起着核心作用。因此，数据基础是 n 个观测值， **X** 包含 p + 1 个预测值，包括截距的 1 个向量。整个数据集(比如说 **Z** 可以表示为成对的 **y** 和 **X** 。在其他符号中，这是![The most popular but often worst method](img/B05113_08_22.jpg)。
...
![The most popular but often worst method](img/B05113_08_04.jpg)
...
![The most popular but often worst method](img/B05113_08_05.jpg)

图 8.5:女性回归系数的自助分布(重复)直方图

我们可以做一些进一步的计算，比如估计回归系数的置信区间。使用函数`boot.ci`可以估计不同类型的置信区间，如偏差修正置信区间、Efron 百分位法置信区间和![The most popular but often worst method](img/B05113_08_23.jpg)法置信区间。对于系数`log(income)`,结果如下:
...
![The most popular but often worst method](img/B05113_08_06.jpg)
...
为了获得自举样本，基于(自举)残差![Bootstrapping by draws from residuals](img/B05113_08_26.jpg)的随机误差被加到预测值![Bootstrapping by draws from residuals](img/B05113_08_25.jpg)上。

理由是 **y** 可以被![Bootstrapping by draws from residuals](img/B05113_08_24.jpg)复制。
...
在下面的代码中，我们估计了回归线，并将其绘制到数据点上。然后，我们抽取一个残差的 bootstrap 样本，并将这些残差添加到拟合值中，即:我们获得新的值![Bootstrapping by draws from residuals](img/B05113_08_27.jpg),将其用作回归问题的输入，以获得新的回归线:
...
![Bootstrapping by draws from residuals](img/B05113_08_07.jpg)

图 8.7:顶部:原始值(圆形圆圈)、拟合值![Bootstrapping by draws from residuals](img/B05113_08_25.jpg)(红色填充圆圈)和原始数据拟合的回归线。底部:拟合值(红色实心圆)，一个由![Bootstrapping by draws from residuals](img/B05113_08_24.jpg)代表的引导样本，以及来自原始值(黑色实线)和引导样本(蓝色虚线)的回归线
...
![Bootstrapping by draws from residuals](img/B05113_08_08.jpg)
...
![Bootstrapping by draws from residuals](img/B05113_08_09.jpg)
...
![Proper variance estimation with missing values](img/B05113_08_28.jpg)
...
设 **X** 为包含缺失值的 **p** 变量和 **n** 独立观测值的样本。关于来自总体参数![Proper variance estimation with missing values](img/B05113_08_30.jpg)的点估计![Proper variance estimation with missing values](img/B05113_08_29.jpg)的置信区间估计如下。请注意，技巧是估算引导样本，而不是原始数据。再次强调，原始样本是你所拥有的最佳信息，这个原始样本的 bootstrap 样本模拟了总体，也考虑了缺失值。

对于![Proper variance estimation with missing values](img/B05113_08_31.jpg)重复:

1.  从原始(非估算)样本 **X** 中抽取一个 bootstrap 样本![Proper variance estimation with missing values](img/B05113_08_32.jpg)。
2.  输入![Proper variance estimation with missing values](img/B05113_08_32.jpg)中缺失的值。
3.  估计参数![Proper variance estimation with missing values](img/B05113_08_33.jpg)。

这导致了对![Proper variance estimation with missing values](img/B05113_08_35.jpg)的 R 个引导复制。然后，该自举复制分布用于确定相对于![Proper variance estimation with missing values](img/B05113_08_29.jpg)的置信区间或标准误差，例如，通过使用 Efron 的百分位数方法，参见[第 7 章](ch07.xhtml "Chapter 7. Resampling Methods")、*重采样方法*。

让我们在实践中这样做。在下面的例子中，使用提到的 bootstrap 来估计具有缺失值的数据的置信区间。简单来说，![Proper variance estimation with missing values](img/B05113_08_30.jpg)应该是一个变量的算术平均值。可重复地，从样本中抽取 bootstrap 样本，估算 bootstrap 样本中的缺失值，最后从 bootstrap 样本中估计算术平均值。我们使用包含缺失值的睡眠数据集:
...
![Proper variance estimation with missing values](img/B05113_08_10.jpg)
...
![Proper variance estimation with missing values](img/B05113_08_11.jpg)
...
![Proper variance estimation with missing values](img/B05113_08_12.jpg)
...
![Proper variance estimation with missing values](img/B05113_08_13.jpg)
...
对于![Proper variance estimation with missing values](img/B05113_08_36.jpg):

1.  输入折叠样本![Proper variance estimation with missing values](img/B05113_08_37.jpg)。
2.  估计估算数据集的参数![Proper variance estimation with missing values](img/B05113_08_30.jpg)。
...
![Bootstrapping in time series](img/B05113_08_014.jpg)
...
因此，在复杂调查抽样中，以已知的包含概率![Bootstrapping in the case of complex sampling designs](img/B05113_08_38.jpg)从 N 大小的总体中对个体进行抽样，最终得到 N 大小的样本。包含概率可能因阶层(总体的分区)而异，甚至可能因抽样框架中的每个个体而异。

对于总体总数，如果个体 I 被抽样，我们有一个无偏估计量![Bootstrapping in the case of complex sampling designs](img/B05113_08_39.jpg)是![Bootstrapping in the case of complex sampling designs](img/B05113_08_42.jpg)和![Bootstrapping in the case of complex sampling designs](img/B05113_08_40.jpg)，如果没有被抽样，则为![Bootstrapping in the case of complex sampling designs](img/B05113_08_41.jpg)。

通常将总体划分为 K 层，从第 K 层抽取![Bootstrapping in the case of complex sampling designs](img/B05113_08_43.jpg)个观测值。然后，可以通过从第 K 层重新采样![Bootstrapping in the case of complex sampling designs](img/B05113_08_44.jpg)个观测值并替换来构建 bootstrap 样本。同样，如果观察值是成组地而不是单独地被抽取到样本中，例如；当绘制家庭图并收集每个家庭成员的信息时，bootstrap 应该对集群而不是个人进行重新采样。这通常被称为天真的引导。几位作者讨论了这种简单方法的局限性(Rao 和 Wu，1988 年)，(Deville 和 s rndal，1992 年)，(Deville，s rndal 和 Sautory，1993 年)。

当我们估算![Bootstrapping in the case of complex sampling designs](img/B05113_08_45.jpg)时，用设计重量![Bootstrapping in the case of complex sampling designs](img/B05113_08_46.jpg)，我们从总体中知道一些特征。例如，我们从样本的一个变量 **x** 中知道总体的真实总数![Bootstrapping in the case of complex sampling designs](img/B05113_08_47.jpg)和![Bootstrapping in the case of complex sampling designs](img/B05113_08_49.jpg)。然后我们用![Bootstrapping in the case of complex sampling designs](img/B05113_08_48.jpg)和![Bootstrapping in the case of complex sampling designs](img/B05113_08_50.jpg)和![Bootstrapping in the case of complex sampling designs](img/B05113_08_51.jpg)搜索新的权重![Bootstrapping in the case of complex sampling designs](img/B05113_08_103.jpg)。如果我们有更多的约束/已知的群体特征，并且如果数据具有聚类结构，这将变得更加复杂。
...
1.  从 **x** 中选择 R 个独立 bootstrap 样本![Bootstrapping in the case of complex sampling designs](img/B05113_08_52.jpg)。
2.  估计每个引导样本的引导复制。
3.  ![Bootstrapping in the case of complex sampling designs](img/B05113_08_53.jpg)。
4.  使用 R 个 bootstrap 复制的标准偏差估算标准误差![Bootstrapping in the case of complex sampling designs](img/B05113_08_56.jpg)，使用![Bootstrapping in the case of complex sampling designs](img/B05113_08_55.jpg)估算![Bootstrapping in the case of complex sampling designs](img/B05113_08_54.jpg)
...
![Bootstrapping in the case of complex sampling designs](img/B05113_08_57.jpg)

人口的平均家庭收入。![Bootstrapping in the case of complex sampling designs](img/B05113_08_59.jpg)是收入的分布函数。

为了估计贫困率，必须考虑抽样权重。首先，我们必须通过以下方式估算贫困线:

![Bootstrapping in the case of complex sampling designs](img/B05113_08_60.jpg)

其中 **x** 是样本中的均衡家庭收入，`wmed`是加权中位数，定义为:

![Bootstrapping in the case of complex sampling designs](img/B05113_08_61.jpg)

如![Bootstrapping in the case of complex sampling designs](img/B05113_08_62.jpg)其中 I 是奇数，如果 I 是偶数，则为![Bootstrapping in the case of complex sampling designs](img/B05113_08_63.jpg)。

贫困率可以借助指数![Bootstrapping in the case of complex sampling designs](img/B05113_08_64.jpg)来估算

![Bootstrapping in the case of complex sampling designs](img/B05113_08_65.jpg)

否则为:

![Bootstrapping in the case of complex sampling designs](img/B05113_08_66.jpg)
...
![A motivating example](img/B05113_08_015.jpg)
...
![A motivating example](img/B05113_08_016.jpg)
...
![A motivating example](img/B05113_08_017.jpg)
...
![A motivating example](img/B05113_08_018.jpg)
...
![A motivating example](img/B05113_08_19.jpg)
...
![A motivating example](img/B05113_08_020.jpg)
...
通常，我们用两组观察值的算术平均值进行比较，并根据算术平均值的显著性差异进行分析。在这种情况下，零假设是具有大小为![The permutation test as a special kind of MC test](img/B05113_08_68.jpg)和![The permutation test as a special kind of MC test](img/B05113_08_69.jpg)的 **x** 和 **y** 向量的![The permutation test as a special kind of MC test](img/B05113_08_67.jpg)。经典(t-)检验的假设由两个总体组成，正态分布，并且方差相等。如果具有算术平均值![The permutation test as a special kind of MC test](img/B05113_08_74.jpg)和![The permutation test as a special kind of MC test](img/B05113_08_75.jpg)的两个样本![The permutation test as a special kind of MC test](img/B05113_08_70.jpg)和![The permutation test as a special kind of MC test](img/B05113_08_71.jpg)是独立的，并且它们的平均值和方差由![The permutation test as a special kind of MC test](img/B05113_08_72.jpg)和![The permutation test as a special kind of MC test](img/B05113_08_76.jpg)给出，那么下面的检验统计量遵循具有自由度![The permutation test as a special kind of MC test](img/B05113_08_73.jpg)的 t 分布:

![The permutation test as a special kind of MC test](img/B05113_08_77.jpg)

对于测试![The permutation test as a special kind of MC test](img/B05113_08_81.jpg)，单边测试的临界区域(替代方案![The permutation test as a special kind of MC test](img/B05113_08_78.jpg)由![The permutation test as a special kind of MC test](img/B05113_08_80.jpg)决定)。对于双面测试

![The permutation test as a special kind of MC test](img/B05113_08_102.jpg)

必须替换为![The permutation test as a special kind of MC test](img/B05113_08_79.jpg)。
...
1.  估计原始样本(有原始类别)中两组均值的绝对差。将此标记为![The permutation test as a special kind of MC test](img/B05113_08_29.jpg)。
2.  通过置换分组/类别从零假设模拟。随机组模拟了均值相等的零假设。计算随机分组结构的平均值之间的绝对差值。将结果表示为![The permutation test as a special kind of MC test](img/B05113_08_83.jpg)。至少重复这个:![The permutation test as a special kind of MC test](img/B05113_08_84.jpg)
3.  p 值由![The permutation test as a special kind of MC test](img/B05113_08_82.jpg)给出。
...
这些组仍然有![The permutation test as a special kind of MC test](img/B05113_08_68.jpg)和![The permutation test as a special kind of MC test](img/B05113_08_69.jpg)观察值，但是它们的观察值是根据组属性随机排列的。这模仿了均值相等的零假设。
...
p 值调整是什么意思？拒绝(当零假设为真时拒绝零假设)所有 k 检验的概率是所有显著性水平的乘积，即:![A Monte Carlo test for multiple groups](img/B05113_08_85.jpg)。如果![A Monte Carlo test for multiple groups](img/B05113_08_86.jpg)和 k = 100，那么拒绝的概率是![A Monte Carlo test for multiple groups](img/B05113_08_88.jpg)。例如，如果从正态分布中抽取数据，随机选择 100 组并进行正态性测试，这也是正确的，因为当从正态分布中抽取数据并进行正态性测试时，每个单一测试将以 0.05 的概率拒绝。因此，显著性检验中的一个常见问题是多重比较倾向于产生虚假的显著差异，即使零假设为真。因此，p 值必须针对多重比较进行调整。
...
1.  抽取 R 个尺寸为![Hypothesis testing using a bootstrap](img/B05113_08_89.jpg)的 bootstrap 样本进行替换。第一批 n1 个观测值现在属于由![Hypothesis testing using a bootstrap](img/B05113_08_90.jpg)表示的样本 1，其余的![Hypothesis testing using a bootstrap](img/B05113_08_69.jpg)个观测值属于第二个样本![Hypothesis testing using a bootstrap](img/B05113_08_93.jpg)。
2.  对于每个引导样本估计值![Hypothesis testing using a bootstrap](img/B05113_08_91.jpg)。
3.  然后，p 值由![Hypothesis testing using a bootstrap](img/B05113_08_92.jpg)给出，其中![Hypothesis testing using a bootstrap](img/B05113_08_29.jpg)是从原始样本估计的。
...
如果 **X** 是具有 *p* 变量、n 个观测值和样本方差![A test for multivariate normality](img/B05113_08_94.jpg)的样本，那么目标是![A test for multivariate normality](img/B05113_08_95.jpg)估计的马氏距离大约为。![A test for multivariate normality](img/B05113_08_96.jpg)以 p 个自由度分布。

单变量安德森-达林检验(安德森和达林 1952 年)的检验统计量定义为:

![A test for multivariate normality](img/B05113_08_97.jpg)

，其中![A test for multivariate normality](img/B05113_08_98.jpg)，F 为正态分布的累积分布函数。![A test for multivariate normality](img/B05113_08_99.jpg)是数据向量的值，按升序排列，![A test for multivariate normality](img/B05113_08_100.jpg)是按降序排列。

这是一个片面的测试和零假设；*样本取自正态分布总体*，如果检验统计量 A 大于临界值，则被拒绝。对于 p、n 和显著性水平的一些情况，临界值是列表值，见 Stephens，1974。

在多元情况下，用![A test for multivariate normality](img/B05113_08_96.jpg)分布和![A test for multivariate normality](img/B05113_08_99.jpg)、![A test for multivariate normality](img/B05113_08_101.jpg)的累积分布函数代替前面公式中的 F，就是马氏距离的![A test for multivariate normality](img/B05113_08_96.jpg)分布的分位数。

安德森-达林检验统计量通常会乘以一个常数，具体取决于样本大小 n。

为了获得 p 值，可以实现蒙特卡罗方法。使用原始数据的均值和协方差对 n x p 大小的数据进行重复随机采样，以模拟零假设(多元正态性)。结果检验统计值(来自 R 人工模拟数据)与原始数据的检验统计值之比反过来充当 p 值。模拟次数越多，结果越稳定。
...
我们模拟多元正态数据，协方差为零，我们重复这个 1000 次，也就是说；蒙特卡洛测试被应用 1000 次。显著性级别设置为![Size of the test](img/B05113_08_86.jpg)。然后我们检查 p 值比`0.05`小多少倍。结果应该是大约的。`0.05`如果测试的尺寸正确:
